    ...
    <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[400] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-6" onClick={onClose}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-[4rem] p-12 max-w-xl w-full shadow-2xl border border-white" onClick={e => e.stopPropagation()}>
              <div className="flex items-center gap-5 mb-8">
                 <div className="p-4 bg-indigo-600 text-white rounded-3xl shadow-lg shadow-indigo-100"><FileJson size={32}/></div>
                 <div>
                    <h3 className="text-3xl font-black text-gray-900 tracking-tight">导入新单元</h3>
                    <p className="text-xs text-indigo-500 font-bold tracking-widest uppercase mt-1">Import External Unit</p>
                 </div>
              </div>
              ...
              <div className="space-y-6">
                 <div>
                    ...
                    <input 
                       ...
                       placeholder="输入单元名称..."
                       className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 outline-none focus:border-indigo-500 transition-all font-bold"
                    />
                 </div>
                 <div>
                    ...
                    <textarea 
                        ...
                        className="w-full h-48 bg-gray-50 rounded-3xl p-6 font-mono text-xs border-2 border-gray-100 focus:border-indigo-500 focus:bg-white transition-all outline-none"
                    />
                 </div>
                 
                 <div className="p-4 bg-indigo-50 rounded-2xl text-xs text-indigo-700 space-y-2">
              ...
              {importError && (
                  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="mt-6 p-4 bg-red-50 border border-red-100 text-red-500 rounded-2xl text-xs font-bold flex items-center gap-3">
                      <AlertCircle size={16}/> {importError}
                  </motion.div>
              )}

              <div className="flex gap-4 mt-10">
                <button onClick={onClose} className="flex-1 py-5 rounded-3xl bg-gray-50 text-gray-400 font-black hover:bg-gray-100 transition-all">取消</button>
                <button onClick={handleImport} className="flex-1 py-5 rounded-3xl bg-indigo-600 text-white font-black hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95">确认导入</button>
              </div>
            </motion.div>
          </motion.div>
        )}
