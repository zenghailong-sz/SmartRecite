...
import { BookOpen, ChevronDown, HelpCircle, Flame, RotateCw, BarChart3, LogIn, LogOut, User as UserIcon } from 'lucide-react';
              ...
              {loading ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="flex flex-col items-end">
                    ...
                    <button onClick={logout} className="text-[10px] font-black text-red-400 hover:text-red-600 uppercase tracking-widest">退出登录</button>
                  </div>
                  {user.photoURL ? (
                    <img src={user.photoURL} alt="Avatar" className="w-10 h-10 rounded-full border-2 border-indigo-100 shadow-sm" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 border-2 border-indigo-200">
                      <UserIcon size={20} />
                    </div>
                  )}
                </div>
